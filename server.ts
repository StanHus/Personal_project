...
app.get("/analysis", async (req, res) => {
  try {
    const progress = await pool.query("SELECT exercise_name, SUM(sets*reps*weight) AS total_weight, COUNT(*) as days_trained, SUM(sets*reps*weight)/count(*) as average_weight, sum(sets)/count(*) as average_sets, sum(reps)/count(*) as average_reps, sum(weight)/count(*) as average_total_weight FROM tracking GROUP BY exercise_name");
